...
const testScraper = async (world) => {
    ...
    page.on('request', (req) => {
        ...
        // const TESTmainFolder = 'TEST_MARGONEM_SCRAPED_DATA'

        // try {
        //     if (!fs.existsSync(TESTmainFolder)) {
        //       fs.mkdirSync(TESTmainFolder);
        //     }
        //   } catch (err) {
        //     console.error(err);
        //   }

        // const TESTserverFolder = `./${TESTmainFolder}/TEST_${world}`;

        // try {
        //   if (!fs.existsSync(TESTserverFolder)) {
        //     fs.mkdirSync(TESTserverFolder);
        //   }
        // } catch (err) {
        //   console.error(err);
        // }
        ...
        for (let pagination = 1; pagination <= TEST.PAGINATION_LOOP; pagination++) {
            ...
            // //Wykonuje Screena strony tabeli
            // console.log('Screenshot...')
            // await page.screenshot({
            //     path: `./${SERVERS[s]}/${pagination}.png`,
            //     fullPage: true
            // })
            ...
            for (let row = 1; row <= TEST.ROWS_LOOP; row++) {
                ...
                // fs.appendFile(`./${TESTmainFolder}/TEST_${world}/data.txt`, JSON.stringify(character) + ';', (err) => {
